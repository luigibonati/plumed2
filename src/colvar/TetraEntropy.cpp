...
class TetraEntropy : public Colvar {
  bool pbc;
  bool serial;
  NeighborList *nl;
  bool invalidateList;
...
public:
  explicit TetraEntropy(const ActionOptions&);
  ~TetraEntropy();
// active methods:
...
PLUMED_REGISTER_ACTION(TetraEntropy,"TETRAENTROPY")

void TetraEntropy::registerKeywords( Keywords& keys ){
...
TetraEntropy::TetraEntropy(const ActionOptions&ao):
...
TetraEntropy::~TetraEntropy(){
  delete nl;
}

void TetraEntropy::prepare(){
  if(nl->getStride()>0){
...
void TetraEntropy::calculate()
{
  ...
  double tetra_entropy=0.;
  vector<Vector> deriv_sum(NumberOfAtoms);
  Tensor virial_sum;
  ...
  for(unsigned int n=rank;n<NumberOfAtoms;n+=stride) {
    ...
    unsigned index_k = list_a[n].index();
    ...
    for(unsigned i=0;i<(neigh_k.size()-1);++i){
      //get relative positions between atom k and neighbor i
      index_i=neigh_k[i];
        ...
        for(unsigned j=i+1;j<neigh_k.size();++j){
          ...
          if ( (d2j=dj[0]*dj[0])<rcut2 && (d2j+=dj[1]*dj[1])<rcut2 && (d2j+=dj[2]*dj[2])<rcut2) {
            ...
            if (!doNotCalculateDerivatives()) {
              ...
              // -- (3) VIRIAL --
              virial[n] += Tensor(di,der_i)+Tensor(dj,der_j);
            }
          }
        }
      }
  ...
  for(unsigned k=0;k<NumberOfAtoms;++k) setAtomsDerivatives(k,3./(2.*NumberOfAtoms) *deriv_sum[k]);
  setValue           (tetra_entropy);
  setBoxDerivatives  (-3./(2.*NumberOfAtoms)*virial_sum);
}
